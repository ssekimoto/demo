...
summary: ADK と Vertex AI Agent Engine を使って、既存環境由来のテレコム/NFV 隣接 Helm チャートを GKE Autopilot 向けにモダナイズする
...
# ADK と Vertex AI Agent Engine を使って、テレコム向け既存環境由来 Helm チャートを GKE Autopilot 向けにモダナイズする
...
このラボでは、テレコムのコアネットワーク部門を支援するプラットフォームエンジニアの立場で作業します。

対象は、**NFV 隣接の運用系コンポーネント**として配布されている、既存環境由来の Helm チャートです。  
このチャートは、権限が緩い従来型 Kubernetes 環境を前提に作られており、テレコム/NFV の現場でありがちな次のようなパターンを含んでいます。
...
こうした設計は、たとえば次のようなコンポーネントで見かけがちです。

- 古い監視サイドカー
- パケットコア周辺の運用補助サービス
- ベンダー提供のトラブルシュート UI
- 広いノードアクセスを前提にした Day-2 運用ユーティリティ
- 一部だけ Kubernetes 化された運用コンソール

このラボでは、**実際のパケット処理を行う VNF/CNF データプレーンを Autopilot に載せることはしません。**  
代わりに、**その周辺にある運用補助コンポーネント**を GKE Autopilot に適合する形へモダナイズします。
...
- テレコム/NFV の顧客に対して、この価値を現実的に説明する方法

### テレコム顧客にとってなぜ重要か

テレコム/NFV の世界では、すべてのワークロードが強く意見を持つマネージド基盤に向くわけではありません。  
ただし、**周辺システム**は非常に相性が良いことが多いです。

- Web UI
- 運用 API
- Mediation / 変換マイクロサービス
- 監査ユーティリティ
- ワークフローヘルパー
- CI/CD の検証ツール
- コンフィグ生成サービス
...
## なぜこのシナリオがテレコム/NFV チームに刺さるのか
Duration: 0:10:00

テレコムのプラットフォームチームは、次のような特徴を持つワークロードを引き継ぐことがよくあります。
...
このラボで修正するチャートは、単なる「既存環境で使われていた YAML」ではありません。  
**テレコムのコアシステム周辺で実際にありそうな運用ユーティリティ**を模したものです。

### 顧客へのメッセージ

このラボで伝えたいメッセージは、次ではありません。

> 「すべてのテレコム CNF を Autopilot に移すべきである」

伝えたい本質は次です。

> 「周辺の運用系資産のかなり大きな部分は、Autopilot 上で安全にモダナイズでき、さらに Agent によってレビュー負荷も下げられる」

この方が、コアネットワーク部門にはずっと自然で信頼されやすい伝え方です。

### GKE と EKS の話し方

このワークショップの目的は、どちらのクラウドが常に優れているかを争うことではありません。

ここで伝えたいのは、より実務的で狭いポイントです。

- このタイプのハンズオンでは、**GKE Autopilot は guarded cluster をすばやく用意しやすい**
...
```text
+------------------------+        +-----------------------------+
|      Cloud Shell       |        |  Vertex AI Agent Engine     |
|  - kubectl             |        |  - managed runtime          |
|  - helm                |        |  - sessions                 |
|  - python / adk        |        |  - scaling                  |
+-----------+------------+        +---------------+-------------+
            |                                     |
            | ローカルで開発・検証                 | 同じ Agent をデプロイ
            v                                     v
+---------------------------------------------------------------+
|                         ADK Agent                              |
|  Tools: read_file, write_file, diff, run_cmd, validate_chart  |
+--------------------------+------------------------------------+
                           |
                           | Helm chart を修正
                           v
